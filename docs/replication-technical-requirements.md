...
`nibabel`, `omegaconf`, `easydict`, `kornia`, `h5py`, `scikit-image`, `einops`, `clip` (OpenAI CLIP, installed from `git+https://github.com/openai/CLIP.git`), `opencv-python-headless` (swapped in for `opencv-python` - the pinned package fails to import here because it needs a system `libGL.so.1` that's absent; headless doesn't need it and the repo never calls any `cv2` GUI/display function, so this is a safe substitution), and `accelerate` (added 2026-09-07, required for the GPU-direct model-loading fix - see §0's update and §3).
...
**Real finding, not anticipated**: the Versatile Diffusion smoke test OOM-killed the VM on first attempt (crashed 2026-09-03, ~24h of runaway CPU before the machine went down) because `get_model()(cfgm)` instantiates the 3.3B-param model in **fp32** (~13.3GB estimated at the time, ~17GB measured directly on 2026-09-07) by default, before the checkpoint is even loaded. Initial fix, applied in the smoke test script: cast to fp16 immediately after instantiation and `mmap=True` on `torch.load`, before `load_state_dict`. This dropped peak RAM from OOM to ~12.5GB in that first pass, though a closer measurement on 2026-09-07 (see below) found it actually still peaks around ~16.3GB - too close to the cap to trust for the full production run. **Superseded 2026-09-07 by a more robust fix - see below.**

**Correction to this section's environment description (2026-09-07)**: "16GB RAM with no swap" is accurate for what's *usable* by CPU-resident allocations, but the container's RAM cap is **not** the GPU-memory cap - that part of the original note was wrong, corrected by direct test. This machine (`victor1`) is an LXC container on a host with a substantially more capable chip (AMD Ryzen AI Max+ 395 APU, unified memory; `rocminfo` reports ~96GB + ~32GB GPU-accessible memory pools). The container itself is hard-capped to exactly 16GiB RAM and 8 CPU threads via a cgroup limit at the container root (confirmed by walking the full cgroup hierarchy - host has 32 threads per `lscpu`). **However, GPU device memory is charged separately and is not subject to this cgroup limit**: a direct 20GB `torch.cuda` allocation left `/sys/fs/cgroup/memory.current` unchanged while `torch.cuda.memory_allocated()` correctly showed 20GB allocated. The VM administrator confirmed this independently (2026-09-07) - the ~96GB GPU pool is real, already accessible from inside this container, and no cluster-side change was needed. **The actionable consequence: code must build/load models directly onto the GPU device rather than staging them through CPU RAM first** (see the updated §3 fix below). The request to raise the container's *CPU-RAM* allocation was accordingly withdrawn / reverted by the admin - 16GiB CPU RAM (no swap) remains the working constraint for anything that must stay CPU-resident (data loading, regression fits, etc.), just not for GPU-resident model weights.
...
**Confirmed, not just anticipated (2026-09-07):** the OOM risk above isn't hypothetical - it's what caused the VM crash documented in §0's update. The originally-documented fix (fp16 cast + mmap, still on CPU) turned out to still peak at ~16.3GB under direct measurement - workable but too close to the cap to trust for the full production run (multiple checkpoints, batch sizes, etc.).

**Robust fix, applied 2026-09-07:** build the model directly on the GPU device instead of CPU, since GPU memory isn't cgroup-limited here (§0 correction above):
```python
