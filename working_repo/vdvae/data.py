...
def imagenet64(data_root):
    # NOTE: brain-diffuser's inference scripts (vdvae_extract_features.py,
    # vdvae_reconstruct_images.py) call set_up_data() -> imagenet64() purely to get
    # H.image_size/H.image_channels and the preprocess_fn closure, which only
    # depend on hardcoded shift/scale constants below in set_up_data() -- the
    # returned train/valid/test arrays are never used by those scripts (the
    # TensorDataset wrappers built from them in set_up_data() are discarded too).
    # The original repo never documents needing OpenAI's ~tens-of-GB ImageNet64
    # training set locally; fall back to tiny dummy arrays when it's absent so
    # inference-only usage doesn't require downloading it.
    try:
        ...
        print('imagenet64-{train,valid}.npy not found locally; using dummy placeholder arrays '
              '(fine for inference-only use, since set_up_data() never returns these to the caller).')
